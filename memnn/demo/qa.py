...
class MemN2N(object):
    ...
    def train(self):
        ...
        # Parse training data
        train_data_path = glob.glob('%s/qa[12]_*_train.txt' % self.data_dir)
        dictionary = {"nil": 0}
        ...
        test_data_path = glob.glob('%s/qa[12]_*_test.txt' % self.data_dir)
    ...
    def predict_answer_for_bot(
        ...
        # Run model to predict answer
        out = self.model.fprop(input_data)
        memory_probs = np.array([self.memory[i].probs[:user_story.shape[1], 0] for i in range(nhops)])
...
def run_console_demo(data_dir, model_file):
    ...
    test_data_path = glob.glob('%s/qa[12]_*_test.txt' % memn2n.data_dir)
...
def run_bot_demo(data_dir, model_file):
    from demo.web import botapp
    botapp.init(data_dir, model_file)
    botapp.run()
...
if __name__ == "__main__":
    ...
    group.add_argument("-bot", "--bot-demo", action="store_true", default=True,
    ...
    if args.train:
        ...
        run_bot_demo(args.data_dir, args.model_file)
